...
## LINK & PREPROCESS
exe touch out.ll.bak
exe $LLVM_LINK *.s out.ll.bak -o out.ll.bak
exe $LLVM_LINK *.ll out.ll.bak -o out.ll.bak
...
if [[ -n "$exclude_file" ]]; then
    ...
    exe rm *.ll
    exe rm *.s
    exe $CLANG $clang_options -O0 -Xclang -disable-O0-optnone -emit-llvm -S $excluded_files
...
if [[ $debug_enabled == false ]]; then
    exe $OPT --passes="strip" out.ll -o out.ll
    echo -e "\xE2\x9C\x94 Debug mode disabled, stripped debug symbols."
fi

exe $OPT --passes="lowerswitch" out.ll -o out.ll

## FuncRetToRef
exe $OPT -load-pass-plugin=$DIR/build/passes/libEDDI.so --passes="func-ret-to-ref" out.ll -o out.ll

echo -e "\n=== ASPIS transformations =========="
## DATA PROTECTION
case $dup in
    0) 
        exe $OPT -load-pass-plugin=$DIR/build/passes/libEDDI.so --passes="eddi-verify" out.ll -o out.ll $eddi_options
        ;;
    1) 
        exe $OPT -load-pass-plugin=$DIR/build/passes/libSEDDI.so --passes="eddi-verify" out.ll -o out.ll $eddi_options -debug
        ;;
    2) 
        exe $OPT -load-pass-plugin=$DIR/build/passes/libFDSC.so --passes="eddi-verify" out.ll -o out.ll $eddi_options
        ;;
esac
echo -e "\xE2\x9C\x94 Applied data protection passes."

$OPT --passes="dce,simplifycfg" out.ll -o out.ll
